...
def evaluate_link_prediction_model(clf, link_examples_test, link_labels_test, get_embedding, binary_operator):
    ...
    score_roc = evaluate_roc_auc(clf, link_features_test, link_labels_test)
    score_auprc = evaluate_auprc(clf, link_features_test, link_labels_test)
    score_f1 = evaluate_f1(clf, link_features_test, link_labels_test)
    return score_roc, score_auprc, score_f1

def evaluate_f1(clf, link_features, link_labels):
    predicted = clf.predict(link_features)
    return f1_score(link_labels, predicted)
...
def evaluate_auprc(clf, link_features, link_labels):
    predicted = clf.predict_proba(link_features)
